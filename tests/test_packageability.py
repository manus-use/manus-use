"""Tests verifying that all manus_agent modules are importable as an installed package.

These tests guard against ``from src.manus_agent.<...>`` imports and module-level
side-effects (network connections, filesystem mutations) that break the package
when installed via ``pip install manus-agent``.

Background
----------
Several tools inside ``src/manus_agent/tools/`` previously used the path
``from src.manus_agent.config import Config``.  That import happens to work when
...
test run (and every ``import manus_agent``) to fail with a network error unless
...
class TestModuleImports:
    ...
    def test_create_lark_document_importable(self):
        ...
        mod = _fresh_import("manus_agent.tools.create_lark_document")
    ...
    def test_search_for_exploits_importable(self):
        ...
        mod = _fresh_import("manus_agent.tools.search_for_exploits")
        assert hasattr(mod, "TOOL_SPEC")
    ...
    def test_get_github_advisory_importable(self):
        ...
        mod = _fresh_import("manus_agent.tools.get_github_advisory")
    ...
    def test_submit_cves_importable_without_mcp_server(self, monkeypatch):
        ...
        with mock.patch(
            ...
            mod = _fresh_import("manus_agent.tools.submit_cves")
    ...
    def test_workflow_agent_importable(self):
        ...
        mod = _fresh_import("manus_agent.multi_agents.workflow_agent")
    ...
    def test_workflow_agent_no_src_path_injection(self):
        ...
        _fresh_import("manus_agent.multi_agents.workflow_agent")
...
def test_no_src_prefix_imports_in_package(tmp_path):
    """No source file under src/manus_agent/ should use 'from src.' imports.
...
def test_no_sys_path_src_injection_in_package():
    """No source file under src/manus_agent/ should call sys.path.insert to add 'src/'.
