...
from .agents import BrowserUseAgent, ManusAgent
...
class EnhancedCLI:
    ...
    def __init__(self, config: Config):
        ...
        self.agent: ManusAgent | None = None
        self.orchestrator: Orchestrator | None = None
        self.browser_agent: BrowserUseAgent | None = None
        ...
        with console.status("Initializing agents...", spinner="dots"):
            try:
                ...
                if mode in ["browser", "auto"]:
                    self.browser_agent = BrowserUseAgent(config=self.config, headless=self.config.browser_use.headless)
        ...
        try:
            ...
            if planner:
                ...
                for i, task_plan in enumerate(plan, 1):
                    ...
                    table.add_row(
                        str(i),
                        task_plan.description[:50] + "..."
                        if len(task_plan.description) > 50
                        else task_plan.description,
...
@click.command()
@click.option(
    "--mode", type=click.Choice(["auto", "single", "multi", "browser"]), default="auto", help="Execution mode"
)
...
def main(mode: str, config: str | None, prompt: str | None, headless: bool, stream: bool, debug: bool, version: bool):
