    ...
    "inputSchema": {
        "json": {
            ...
            "properties": {
                ...
                "technical_details": {
                    "type": "string",
                    "description": "Provide concise technical details for the vulnerability using authoritative sources such as a GitHub Security Advisory, the original author's write-up, vendor advisory, or NVD entry. Focus on vulnerability mechanics, affected components, exploitation prerequisites or scenarios, impact, and detection guidance. Structure this field with these exact Markdown subsection headers where the corresponding content is present: `### Detection guidance`, `### Exploitability Analysis`, `### Expected impact`, and `### Affected conditions`. Prefix those subsection headers with `### ` exactly, and do not render those labels as plain text or bold-only labels. Avoid one large paragraph; use short paragraphs separated by blank lines, and use bullet points when listing components, prerequisites, impacts, or detection indicators. Use Markdown syntax only when needed, especially the required `### ` subsection headers and inline code for files, functions, variables, commands, CVE/CWE identifiers, or other technical names; avoid decorative formatting such as bolding or bold-only section labels.",
                },
                "cwe_info": {
                    "type": "string",
