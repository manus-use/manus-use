...
from manus_agent.sandbox import DockerSandbox
...
class CodeExecutor:
    ...
    async def _get_sandbox(self) -> DockerSandbox | None:
    ...
    async def execute_python(self, code: str, timeout: int | None = None) -> tuple[str, str, int]:
        """Execute Python code."""
    ...
    async def execute_bash(self, command: str, timeout: int | None = None) -> tuple[str, str, int]:
...
def get_executor(config: Config | None = None) -> CodeExecutor:
...
async def code_execute(
    ...
    try:
        ...
        return {"stdout": stdout, "stderr": stderr, "exit_code": exit_code, "error": stderr if exit_code != 0 else None}
        ...
        return {"stdout": "", "stderr": str(e), "exit_code": -1, "error": f"Execution failed: {str(e)}"}
...
def code_execute_sync(
    ...
    try:
        return loop.run_until_complete(code_execute(code, language, timeout))
    finally:
        loop.close()
