    ...
    if platform.system() == "Darwin":
        ...
        hosts.extend(
            [
                ...
                (f"unix://{home}/Library/Containers/com.docker.docker/Data/docker.raw.sock", "Docker Desktop (legacy)"),
            ]
        )
...
def _check_docker_context() -> tuple[str, str] | None:
...
def get_docker_client(timeout: int = 10) -> docker.DockerClient:
    ...
    raise DockerConnectionError(
        message="Failed to connect to Docker daemon", diagnosis=diagnosis, remediation=remediation
    )
...
4. Check Docker logs for errors"""
        ...
        diagnosis = "Docker daemon connection failed. Errors encountered:\n" + "\n".join(f"  - {e}" for e in errors)
