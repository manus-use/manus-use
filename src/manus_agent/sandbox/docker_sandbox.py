...
class DockerSandbox:
    ...
    def _start_sync(self):
        ...
        self.container = docker_retry(
            ...
            lambda: self.client.containers.run(
                ...
                labels={
                    "manus_agent.component": "docker_sandbox",
                    "manus_agent.sandbox_uid": self.container_name,
                },
            ),
        )
    ...
    async def execute_code(
        ...
        try:
            ...
            await loop.run_in_executor(None, self._copy_to_container, temp_file, container_path)
    ...
    async def execute_command(
        ...
        try:
            result = await asyncio.wait_for(loop.run_in_executor(None, self._execute_sync, command), timeout=timeout)
            return result
        except asyncio.TimeoutError:
            # Try to stop the execution
            try:
                self.container.exec_run("pkill -9 -f " + command.split()[0])
            except Exception:
                pass
    ...
    def _copy_to_container(self, local_path: str, container_path: str):
        ...
        self.container.put_archive(Path(container_path).parent, tar_stream.getvalue())
