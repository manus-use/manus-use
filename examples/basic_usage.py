...
def example_manus_agent():
    ...
    # Use the agent
    response = agent(
        "Create a Python script that generates the Fibonacci sequence up to n=10 and save it to fibonacci.py"
    )
    print(response)
...
def example_browser_agent():
    ...
    response = agent("Search for information about the latest Python version and summarize the key new features")
    print(response)
...
def example_data_analysis():
    ...
    # Analyze data
    response = agent("Create sample sales data for the last 12 months and generate a trend analysis with visualization")
    print(response)
